...
$staticPages = [
    ...
    '/recipes.php',
    '/community.php',
    '/community_market.php',      // Nauja: Turgelis
    '/community_discussions.php', // Nauja: Forumas
    '/delivery.php',              // Nauja: Pristatymas
    '/returns.php',               // Nauja: Grąžinimas
    '/contact.php',
    '/faq.php',
    '/about.php'
];
...
// 5. Bendruomenės Turgelis (Skelbimai)
$stmt = $pdo->query("SELECT id, created_at FROM community_listings WHERE status IN ('active', 'sold') ORDER BY created_at DESC");
while ($row = $stmt->fetch()) {
    // Turgelio URL formatas pagal community_market.php
    $url = $baseUrl . '/community_listing.php?id=' . $row['id'];
...
// 6. Bendruomenės Forumas (Diskusijos)
$stmt = $pdo->query("SELECT id, created_at FROM community_threads ORDER BY created_at DESC");
while ($row = $stmt->fetch()) {
    // Forumo URL formatas pagal community_discussions.php
    $url = $baseUrl . '/community_thread.php?id=' . $row['id'];
