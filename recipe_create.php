...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $selectedCatIds = $_POST['categories'] ?? []; // Kategorijų masyvas
    ...
    if ($title === '' || $body === '' || $summary === '') {
        $errors[] = 'Užpildykite pavadinimą, santrauką ir aprašymą.';
    }

    $imagePath = '';
    $uploaded = uploadImageWithValidation($_FILES['image'] ?? [], 'recipe_', $errors, 'Įkelkite recepto nuotrauką.');
    if ($uploaded) {
        $imagePath = $uploaded;
    }
    ...
    if (!$errors) {
        try {
            ...
            // 1. Įrašome receptą su naujais laukais
            $stmt = $pdo->prepare('INSERT INTO recipes (title, summary, author, image_url, body, visibility, is_visible, publish_date, seo_keywords) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
            $stmt->execute([$title, $summary, $author, $imagePath, $body, $visibility, $isVisible, $publishDateDb, $seoKeywords]);
            ...
            if (!empty($selectedCatIds)) {
                ...
                foreach ($selectedCatIds as $catId) {
                    $relStmt->execute([$recipeId, (int)$catId]);
                }
            }
            ...
            logError('Recipe creation failed', $e);
...
<head>
  ...
  <title>Kurti receptą | Cukrinukas</title>
...
<body>
  ...
  <div class="wrapper">
    <div class="card">
      ...
      <p style="margin:0 0 14px; color:#444;">Pasidalinkite sveiku receptu.</p>
      ...
      <form method="post" enctype="multipart/form-data" onsubmit="return syncBody();">
        ...
        <label for="author">Autorius (šaltinis)</label>
        <input id="author" name="author" type="text" value="<?php echo htmlspecialchars($author); ?>" placeholder="pvz. Cukrinukas virtuvė">
        ...
        <label for="body-editor">Recepto aprašymas</label>
        ...
        <input id="seo_keywords" name="seo_keywords" type="text" value="<?php echo htmlspecialchars($seoKeywords); ?>" placeholder="diabetas, receptai, mityba...">

        <label for="visibility">Matomumas</label>
        <select id="visibility" name="visibility">
          <option value="public" <?php echo $visibility === 'public' ? 'selected' : ''; ?>>Visiems matomas</option>
          <option value="members" <?php echo $visibility === 'members' ? 'selected' : ''; ?>>Tik prisijungusiems</option>
        </select>

        <button type="submit">Sukurti receptą</button>
      </form>
      
      <div style="margin-top: 16px;">
        <a href="/recipes.php" style="text-decoration:underline;">Atšaukti</a>
      </div>
    </div>
  </div>
  <script>
    ...
    function syncBody() {
      ...
      if (!content || content === '<br>') {
        alert('Prašome užpildyti recepto aprašymą.');
        return false;
      }
